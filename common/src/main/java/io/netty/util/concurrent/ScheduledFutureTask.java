...
/**
 * ScheduledFutureTask继承自PromiseTask, 并实现了ScheduledFuture接口.
 * 而ScheduledFuture接口是从java.util.concurrent.ScheduledFuture继承, 再往上是Delayed接口,
 * 这里有对于定时任务而言最重要的一个方法getDelay().
 *
 * @param <V>
 */
...
final class ScheduledFutureTask<V> extends PromiseTask<V> implements ScheduledFuture<V>, PriorityQueueNode {
     ...
     * 取当前时间点
     *
     * nanoTime()方法并没有直接调用System.nanoTime(),
     * 而是先通过常量START_TIME中保存着类装载时的时间值,
     * 然后nanoTime()方法中用每次取当前时间减去这个START_TIME, 返回其差值.
     *
     * @return
     */
