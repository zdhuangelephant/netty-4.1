...
public class DefaultPromise<V> extends AbstractFuture<V> implements Promise<V> {
    ...
    public Promise<V> await() throws InterruptedException {
        ...
        synchronized (this) {
            while (!isDone()) {
                // 每一次调用 AbstractFuture#get() 获取Promise执行结果的时候都会调用 incWaiters() 方法 ++waiters
                incWaiters();
                try {
                    wait();
                } finally {
                    // 执行完后调用 decWaiters() 进行 --waiters。
                    decWaiters();
                }
            }
        }
    ...
    /**
     * 检查执行 Promise 任务的线程是否是当前线程，如果是当前线程再调用 await() 就会发生死锁的情况，抛出 BlockingOperationException 异常。
     * 就相当于获取结果和执行都是当前线程，自己调用 await() 方法把自己阻塞掉了。
     */
    ...
    private void notifyListeners() {
        ...
        // 不一致则将任务封装成 Runnable 提交到任务队列中等待执行。
    ...
    private void notifyListenersNow() {
        ...
        for (;;) {
            ...
            // 如果不为null，说明在执行过程中又有新的 listeners 添加进来，继续 for 循环执行
            synchronized (this) {
    ...
    private boolean setValue0(Object objResult) {
        ...
        if (RESULT_UPDATER.compareAndSet(this, null, objResult) ||
            ...
            /**
             * 调用checkNotifyWaiters() 方法进行唤醒所有 调用 DefaultPromise.get() 获取结果的线程。
             */
            if (checkNotifyWaiters()) {
                notifyListeners();
            }
